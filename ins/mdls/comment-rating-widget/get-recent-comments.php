<?php
/*
Plugin Name: Comment Rating Widget
Version: 2.0.6.7
Plugin URI: http://wealthynetizen.com/comment-rating-widget
Author: Krischan Jodies, Bob King
Author URI: http://wealthynetizen.com
Description: Display Comment Rating along with the most recent comments/trackbacks in the sidebar. Built upon Get Recent Comment widget. Visit <a href="options-general.php?page=get-recent-comments.php">Options/Comment Rating Widget</a> after activation of the plugin.
...
<h2><?php _e('Comment Rating Widget') ?></h2>

<p>
<b>
The <a href="http://wealthynetizen.com/donate/">Comment Rating Widget Pro</a>
can show comments ordered by their Rating in the last X days.
</b>
</p>
...
<td rowspan="6"><pre><div style='font-size: 10px; border-left: 1px solid; margin: 0px;'> %comment_rating  - Comment Rating images and votes.
...
<td rowspan="3"><pre><div style='font-size: 10px; border-left: 1px solid; margin: 0px;'> %comment_rating  - Comment Rating images and votes.
 %comment_excerpt - Shortened comment.
 %comment_link    - Link to the comment.
 %comment_author  - Name left by the commenter
 %comment_date    - Date of comment
...
<span class="code">ins/themes/default/sidebar.php</span>
...
function kjgrc_get_option($section,$option_name)
{
	...
	if ($kjgrc_options == NULL || $kjgrc_options['version'] != $version) 
	{
		$default_options = array (
			...
			"grc_format" => "<li><a href=\"%comment_link\" title=\"%post_title, %post_date\">%comment_author</a>: %comment_excerpt %comment_rating</li>",
			...
			"grt_format" => "<li><a href=\"%comment_link\" title=\"Trackback to &quot;%post_title&quot;: %comment_excerpt\">%comment_author</a>: %trackback_title</li> %comment_rating",
			"misc_exclude_cat" => "",
			...
			"grt_ignore_ip" => "",
			"gravatar_rating" => 0,
			"grc_sidebar_title" => "Comment Rating Widget",
		...
		// todo: check new params
	{
		    ...
		    ($function == 'grt'))
		{
			if (!empty($_POST['reset_template'])) {
				if ($function == 'grc') {
					kjgrc_set_option('grc_format','<li><a href="%comment_link" title="%post_title, %post_date">%comment_author</a>: %comment_excerpt %comment_rating</li>');
				...
				if ($function == 'grt') {
					kjgrc_set_option('grt_format','<li><a href="%comment_link" title="Trackback to &quot;%post_title&quot;: %comment_excerpt">%comment_author</a>: %trackback_title %comment_rating</li>');
				}
...
{
	add_options_page('Comment Rating Widget Plugin', 'Comment Rating Widget', 8, 'get-recent-comments.php','kjgrc_options_page');
}
...
function kjgrc_get_comments2 ($requested_comment_type,$caller)
{
	...
	foreach ($fetched_comments as $comment)
	{
      ...
      /*  The following doesn't handle paged comments well.
      ...
      */
      ...
      $comment_rating ='';
      if(function_exists('ckrating_display_sidebar')) {
         $comment_rating = ckrating_display_sidebar($comment->comment_ID);
      }
		...
		$output = str_replace("%comment_rating",  $comment_rating, $output);
...
function widget_kj_get_recent_comments_init() {
	...
	register_sidebar_widget('Comment Rating Widget', 'widget_get_recent_comments');
	register_widget_control('Comment Rating Widget', 'widget_get_recent_comments_control', 300, 100);
}
